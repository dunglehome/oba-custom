...
var oba_where_standard_trip = function(data)
{
	...
	/**
	 * Create map markers for a stop and a pop-up info window for display when
	 * clicked
	 */
	...
	/**
	 * Trip details handler that displays the shape of the trip, if applicable,
	 * and creates markers for all the stops along the trip
	 */
	var tripDetailsHandler = function(tripDetails)
	{
		if (tripDetails.trip)
		{
			...
			if (trip.shapeId) OBA.Api.shape(trip.shapeId, shapeHandler);
		}
		...
		if (tripDetails.status && tripDetails.status.lastKnownLocation)
		{
			...
			vehicleLocationMarker = new google.maps.Marker({
				position : location,
				map : map,
				clickable : false
			});
