...
public class ArrivalsAndDeparturesPresentaion
{
	...
	private boolean						_showArrivalsCustom		= false;
	private boolean						_showDeparturesCustom	= false;
	...
	{
		_showArrivalsCustom = showArrivalsCustom;
	}
	
	public boolean isShowArrivalsCustom()
	{
		return _showArrivalsCustom;
	}
	
	public void setShowDeparturesCustom(boolean showDeparturesCustom)
	{
		_showDeparturesCustom = showDeparturesCustom;
	}
	
	public boolean isShowDeparturesCustom()
	{
		return _showDeparturesCustom;
	}
	...
	public String getStatusLabel(ArrivalAndDepartureBean pab)
	{
		...
		if (predicted > 0)
		{
			...
			{
				if (pastTense)
					return _showArrivals ? _messages.arrivedEarly(minutes) : _messages.departedEarly(minutes);
				else
			...
			{
				if (pastTense)
					return _showArrivals ? _messages.arrivedOnTime() : _messages.departedOnTime();
				else
			...
			{
				if (pastTense)
					return _showArrivals ? _messages.arrivedLate(minutes) : _messages.departedLate(minutes);
				else
		...
		else
		{
			...
			else if (_showArrivalsCustom)
				return _messages.scheduledArrivalCustom();
			else if (_showDeparturesCustom)
				return _messages.scheduledDepartureCustom();
			else
	...
	{
		return _showArrivals ? pab.hasPredictedArrivalTime() : pab.hasPredictedDepartureTime();
	}
	
	private long getPredictedTime(ArrivalAndDepartureBean pab)
	{
		return _showArrivals ? pab.getPredictedArrivalTime() : pab.getPredictedDepartureTime();
	}
	
	private long getScheduledTime(ArrivalAndDepartureBean pab)
	{
		return _showArrivals ? pab.getScheduledArrivalTime() : pab.getScheduledDepartureTime();
	}
